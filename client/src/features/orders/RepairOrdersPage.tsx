...
export default function RepairOrdersPage() {
  ...
  const discountTimerRef = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
  ...
  const sendWhatsApp = () => {
    ...
    // Convert Eastern Arabic/Persian numerals (٠-٩ and ۰-۹) to standard Western numerals (0-9)
    const arabicPersianMap: Record<string, string> = {
      '٠': '0', '١': '1', '٢': '2', '٣': '3', '٤': '4', '٥': '5', '٦': '6', '٧': '7', '٨': '8', '٩': '9',
      '۰': '0', '۱': '1', '۲': '2', '۳': '3', '۴': '4', '۵': '5', '۶': '6', '۷': '7', '۸': '8', '۹': '9'
    };
    let phone = (selectedOrder.customerPhone || '')
      .replace(/[٠-٩۰-۹]/g, (d) => arabicPersianMap[d] || d);

    // Remove ALL non-digit characters (spaces, dashes, +, parens, dots, non-breaking spaces)
    ...
    // Handle common mistake of writing +20 010xxxxxxxx (which becomes 20010xxxxxxxx)
    if (phone.startsWith('200') && phone.length === 13) {
      phone = '20' + phone.substring(3);
    }

    // Now normalize to WhatsApp international format (no + sign)
    if (phone.startsWith('20') && phone.length === 12) {
      // Already correct: 201012345678
    } else if (phone.startsWith('0') && phone.length === 11) {
      // Egyptian format with leading zero: 01012345678
      phone = '20' + phone.substring(1);
    } else if (phone.length === 10) {
      // Without leading zero: 1012345678
      phone = '20' + phone;
    }

    // Show alert if number format looks wrong
    if (phone.length !== 12) {
      alert(`تحذير: رقم الهاتف "${selectedOrder.customerPhone}" قد يكون بصيغة غير صحيحة.\nتأكد من أن الرقم مسجل بالصيغة: 01012345678`);
    }

    const servicesList = selectedOrder.services.length > 0
      ? selectedOrder.services.map((s) => `  - ${s.serviceName}: ${s.price.toLocaleString('ar-EG')} ج.م`).join('\n')
      : '  لا توجد خدمات مسجلة';

    const partsList = selectedOrder.parts.length > 0
      ? '\n🔩 *قطع الغيار:*\n' + selectedOrder.parts.map((p) => `  - ${p.itemName} (${p.quantity}x): ${p.totalPrice.toLocaleString('ar-EG')} ج.م`).join('\n')
      : '';

    const discountLine = selectedOrder.discountPercentage > 0
      ? `\n🏷️ خصم (${selectedOrder.discountPercentage}%): ${selectedOrder.discountAmount.toLocaleString('ar-EG')} ج.م -`
      : '';

    const paymentStatus = selectedOrder.remainingAmount > 0
      ? `⚠️ *المتبقي:* ${selectedOrder.remainingAmount.toLocaleString('ar-EG')} ج.م`
      : '✅ *تم السداد الكامل*';

    const message =
`🔧 *فاتورة مركز مكانك سيرفس*
━━━━━━━━━━━━━━━━━
رقم الأمر: *${selectedOrder.orderCode}*
العميل: ${selectedOrder.customerName}
السيارة: ${selectedOrder.vehicleMakeModel}
رقم اللوحة: *${selectedOrder.vehiclePlate}*
━━━━━━━━━━━━━━━━━
✅ *الخدمات:*
${servicesList}${partsList}${discountLine}
━━━━━━━━━━━━━━━━━
💰 *الإجمالي:* ${selectedOrder.totalAmount.toLocaleString('ar-EG')} ج.م
💳 *المدفوع:* ${selectedOrder.paidAmount.toLocaleString('ar-EG')} ج.م
${paymentStatus}
━━━━━━━━━━━━━━━━━
شكراً لثقتكم في مكانك سيرفس 🚗`;
        ...
        <div className="fixed inset-0 z-50 flex flex-col overflow-y-auto" style={{ background: 'var(--bg-primary)' }}>
          ...
          <div className="flex justify-between items-center px-6 md:px-8 py-4 no-print sticky top-0 z-10 backdrop-blur-xl" style={{ background: 'color-mix(in srgb, var(--bg-card) 85%, transparent)', borderBottom: '1px solid var(--border-color)' }}>
            ...
            <div className="flex gap-2">
              <button onClick={sendWhatsApp} className="mk-btn mk-btn-ghost text-xs flex" style={{ color: '#25D366' }}>
                <FaWhatsapp size={16} />
                <span>واتساب</span>
              </button>
              <button onClick={printInvoice} className="mk-btn mk-btn-ghost text-xs flex">
                <Printer size={16} />
                <span>طباعة</span>
              </button>
